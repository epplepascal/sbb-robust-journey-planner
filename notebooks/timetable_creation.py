...
# %% [markdown]
# We are going to construct the timetable dataframe based on the week of the 3rd May 2023.
...
# %% [markdown]
# ## Loading the stops in a cercle of 30km around Zurich HB

# %% language="spark"
# #Loading the file previously preprocessed
# df_zurich = spark.read.csv('/user/romanato/work/zurich.csv', header=True, encoding='utf8')
...
# zurich_18km = df_zurich.filter(df_zurich['center_dist'] <= 18).select('unknown', 'stop_id', 'stop_name')
#
...
# %% [markdown]
# Once this step is done, you should copy the created timetable to the local filesystem. To do so, run the following command line:
#
# ```hdfs dfs -copyToLocal /user/nmuenger/work/timetable_zh.csv /data/```
